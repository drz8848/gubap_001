...
# 控制变量（用于调试，限制爬取数量）
# 当对应的控制变量加1递增量达到设定值时，退出相应循环
控制变量1_限制 = 999999  # 股票数量控制（处理多少只股票后停止）
控制变量2_限制 = 999999  # 每页数量控制（最多爬取多少页）
控制变量3_限制 = 999999  # 单页文章数量控制（每页最多爬取多少篇文章）
...
def 检查帖子是否已存在(conn, post_url: str) -> bool:
    """检查帖子URL是否已存在于数据库"""
    cursor = conn.cursor()
    cursor.execute("SELECT 1 FROM stock_guba_post WHERE post_url = ? LIMIT 1;", (post_url,))
    return cursor.fetchone() is not None


def 获取已爬取的帖子URL集合(conn) -> set:
    """批量获取已爬取的帖子URL，减少数据库查询"""
    cursor = conn.cursor()
    cursor.execute("SELECT post_url FROM stock_guba_post;")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 预加载已爬取的帖子URL集合（断点续爬核心）
        print("\n🔄 加载已爬取帖子列表用于断点续爬...")
        已爬帖子URL集合 = 获取已爬取的帖子URL集合(数据库连接)
        print(f"✅ 已加载{len(已爬帖子URL集合)}条已爬取记录")
        ...
        try:
            ...
            # 步骤3：遍历任务列表中的股票代码
            目标列 = "stock_code"
            for 任务索引, 任务行 in 任务数据框.iterrows():
                股票代码 = 任务行[目标列]
                已爬页数 = 任务行.get("crawl_page", 0)
                爬取状态 = 任务行.get("crawl_status", "running")
                ...
                print(f"\n{'='*60}")
                print(f"🔹 开始处理股票：{股票代码}（已爬：{已爬页数}页，状态：{爬取状态}）")
                print(f"{'='*60}")
                ...
                try:
                    ...
                    if not 总页数:
                        ...
                        打开谷歌网页.关闭单个标签页(主标签页)
                        continue
                    print(f"📌 股票{股票代码}总页数：{总页数}，当前已爬：{已爬页数}页")

                    # 检查是否已全部完成
                    if 已爬页数 >= 总页数:
                        print(f"✅ 股票{股票代码}已全部爬取完成（{已爬页数}/{总页数}页），只检查最新一页是否有新帖")
                        # 重置起始页码为最后页，以检查新帖子
                        已爬页数 = 总页数 - 1

                    # 3.2 从后往前遍历页码（总页数→1），跳过已爬完的页
                    控制变量2 = 0
                    控制变量3 = 0  # 每切换新页，文章计数器重置

                    # 计算起始页码：总页数 - 已爬页数 + 1
                    # 例如：总3345页，已爬2页（已爬3345和3344），从第3344页开始（多检查一页）
                    起始页码 = 总页数 - 已爬页数 + 1
                    if 起始页码 < 1:
                        起始页码 = 1
                        print(f"🚀 从第1页开始爬取")
                    else:
                        print(f"🚀 从第{起始页码}页开始（多检查最后已爬页面是否有新帖子）")

                    for 页码 in range(起始页码, 0, -1):
                        控制变量2 += 1
                        控制变量3 = 0  # 每页重置文章计数器
                        ...
                        try:
                            ...
                            详情页URL列 = "详情页URL"
                            原始文章数 = len(文章数据框)
                            文章数据框 = 文章数据框[~文章数据框[详情页URL列].isin(已爬帖子URL集合)]
                            新增文章数 = len(文章数据框)
                            跳过文章数 = 原始文章数 - 新增文章数

                            if 跳过文章数 > 0:
                                print(f"🔄 页面{页码}：跳过{跳过文章数}条已爬记录，需爬取{新增文章数}条新文章")

                            if 文章数据框.empty:
                                print(f"✅ 页面{页码}：所有文章已爬取，跳过")
                                # 仍然需要更新爬取页数，因为该页已完整爬取
                            ...
                            for 详情页链接 in 文章数据框[详情页URL列]:
                                ...
                                # 调试：限制单页文章数量
                                if 控制变量3 >= 控制变量3_限制:
                                    print(f"🛑 单页文章数量达上限（{控制变量3_限制}篇），停止处理当前页")
                                    break
                        ...
                        # 调试：限制爬取页数
                        if 控制变量2 >= 控制变量2_限制:
                            print(f"🛑 页数达上限（{控制变量2_限制}页），停止处理当前股票")
                            break
                ...
                # 调试：限制爬取股票数量
                if 控制变量1 >= 控制变量1_限制:
                    print(f"🛑 股票数量达上限（{控制变量1_限制}只），结束程序")
                    break
