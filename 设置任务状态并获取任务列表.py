...
def 初始化并读取爬取任务列表(从股票基础表补充任务: bool = True) -> pd.DataFrame:
    ...
    CREATE TABLE IF NOT EXISTS stock_crawl_task_log (
        ...
        task_name VARCHAR(100) NOT NULL COMMENT '爬取任务名称（如20251230_guba_crawl_000001）',
        stock_code VARCHAR(20) NOT NULL COMMENT '关联股票代码',
        guba_list_url VARCHAR(255) NOT NULL COMMENT '股吧列表页相对路径',
        crawl_page INT(11) DEFAULT 0 COMMENT '已爬取页数',
        last_crawl_post_time DATETIME DEFAULT NULL COMMENT '最后爬取帖子时间',
        crawl_status VARCHAR(20) DEFAULT 'running' COMMENT '任务状态',
        error_msg TEXT DEFAULT NULL COMMENT '错误信息',
        crawl_start_time DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '任务开始时间',
        crawl_complete_time DATETIME DEFAULT NULL COMMENT '任务完成时间',
        PRIMARY KEY (id),
        KEY idx_stock_code_task (stock_code, task_name),
        KEY idx_crawl_status (crawl_status)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='爬取任务状态管理表';
    """
    ...
    try:
        ...
        cursor = conn.cursor()
        print("成功连接MySQL数据库，开始处理任务状态管理表")
...
def 重置所有爬取状态(清空任务表: bool = True, 重置股票基础表: bool = True) -> bool:
    ...
    try:
        ...
        # 3. 清空任务表（TRUNCATE会删除所有数据并重置自增ID）
        if 清空任务表:
